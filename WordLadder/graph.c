...
Graph *CreateGraph(int N)
{
	...
	G->NamesTree = (AVLNode*)mymalloc(N * sizeof(AVLNode));
	if (G->NamesTree == NULL)
	{
		printf("\n**Error in CreateGraph: malloc failed to allocate\n\n");
		exit(-1);
...
//
AVLNode* AddVertex(Graph *G, char *name)
{
...
{
	// use the tree to look up the vertex by name
	AVLNode *cur = NULL;
	cur = G->NamesTree;
	while (cur != NULL)
	{
		if (strcmp(Name, cur->value.Word) == 0)
		{
			return cur->value.Vertex;
		}
		else if (strcmp(Name, cur->value.Word) < 0)
		{
			cur = cur->left;
		}
		else
